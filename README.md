...
#### windows
Make a batch file, such as `phoebus.bat` and set the path as `%WIDGET_JAR%` and phoebus jar as `%PH_JAR%`
```powershell
...
#### linux
Make a bash script, such as `phoebus.sh` and set the the path to the jar as `$WIDGET_JAR`, and phoebus jar would be `$PH_JAR`
```
java -Dorg.csstudio.display.builder.representation.spi.WidgetRepresentationsService=FINE -Dfile.encoding=UTF-8 -cp "$PH_JAR:$WIDGET_JAR" org.phoebus.product.Launcher
```
